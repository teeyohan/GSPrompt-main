...
class TaskPrompter(nn.Module):
    ...
    def __init__(self, p, select_list, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        self.prompts_len = self.task_no*self.prompt_len
        if self.task_no == 4: # nyud
            self.seg_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.dep_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.edg_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.nor_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            trunc_normal_(self.seg_mu, mean=0., std=1.)
            trunc_normal_(self.dep_mu, mean=0., std=1.)
            trunc_normal_(self.edg_mu, mean=0., std=1.)
            trunc_normal_(self.nor_mu, mean=0., std=1.)
        elif self.task_no == 5: # pascal
            self.seg_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.hum_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.sal_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.edg_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.nor_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            trunc_normal_(self.seg_mu, mean=0., std=1.)
            trunc_normal_(self.hum_mu, mean=0., std=1.)
            trunc_normal_(self.sal_mu, mean=0., std=1.)
            trunc_normal_(self.edg_mu, mean=0., std=1.)
            trunc_normal_(self.nor_mu, mean=0., std=1.)
        elif self.task_no == 2: # cityscapes
            self.seg_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            self.dep_mu = nn.Parameter(torch.zeros(self.prompt_len, embed_dim))
            trunc_normal_(self.seg_mu, mean=0., std=1.)
            trunc_normal_(self.dep_mu, mean=0., std=1.)
        else:
            raise ValueError('task_no is not satisfied!')

        self.logvar = nn.Parameter(torch.ones(self.prompts_len, embed_dim))
        trunc_normal_(self.logvar, mean=0., std=1.)
    ...
    def forward(self, x):
        ...
        # put prompts in it
        if self.task_no == 4:
            seg_mu = self.seg_mu[None].expand(x.shape[0], -1, -1)
            dep_mu = self.dep_mu[None].expand(x.shape[0], -1, -1)
            edg_mu = self.edg_mu[None].expand(x.shape[0], -1, -1)
            nor_mu = self.nor_mu[None].expand(x.shape[0], -1, -1)
            task_mus = torch.cat((seg_mu,dep_mu,edg_mu,nor_mu), dim=1)
        elif self.task_no == 5:
            seg_mu = self.seg_mu[None].expand(x.shape[0], -1, -1)
            hum_mu = self.hum_mu[None].expand(x.shape[0], -1, -1)
            sal_mu = self.sal_mu[None].expand(x.shape[0], -1, -1)
            edg_mu = self.edg_mu[None].expand(x.shape[0], -1, -1)
            nor_mu = self.nor_mu[None].expand(x.shape[0], -1, -1)
            task_mus = torch.cat((seg_mu,hum_mu,sal_mu,edg_mu,nor_mu), dim=1)
        elif self.task_no == 2:
            seg_mu = self.seg_mu[None].expand(x.shape[0], -1, -1)
            dep_mu = self.dep_mu[None].expand(x.shape[0], -1, -1)
            task_mus = torch.cat((seg_mu,dep_mu), dim=1)
        else:
            raise ValueError('task_no is not satisfied!')

        logvar = self.logvar[None].expand(x.shape[0], -1, -1)
        task_prompts = task_mus + torch.rand_like(logvar, device=logvar.device)*logvar
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx + 1 in self.select_list: 
                ...
                _cur_task_fea, info = self.cal_task_feature(x, task_mus, attn_weight, il, info)
        ...
        # task_mus = self.norm(task_mus)
        task_prompts = self.norm(task_prompts)

        return task_fea, task_mus, task_prompts, info

    def cal_task_feature(self, x, task_mus, attn_weight, il, info):
        ...
        for t_idx, task in enumerate(self.p.TASKS.NAMES):
            ...
            # take the mu corresponding to the task.
            cur_task_mu = task_mus[:, t_idx*self.prompt_len:(t_idx+1)*self.prompt_len, :]
            cur_task_mu = torch.mean(cur_task_mu, dim=1, keepdim=True)
            cur_x = rearrange(x + cur_task_mu, 'b (h w) c -> b c h w', h=self.resolution[0], w=self.resolution[1])
            ...
            for hea in range(nheads):
                ...
                cur_task_fea.append(cur_head_attn * cur_x[:, head_channel_no*hea:head_channel_no*(hea+1), :, :])
            cur_task_fea = torch.cat(cur_task_fea, dim=1) + cur_x
            ...
            for h_idx in range(nh):
                ...
                for w_idx in range(nw):
                    _patch = cur_x[:, :, h_idx*win_h:(h_idx+1)*win_h, w_idx*win_w:(w_idx+1)*win_w]
            ...
            cur_task_fea = torch.cat(cur_task_fea, dim=2) + cur_x
