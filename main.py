...
parser.add_argument('--config_exp', default='configs/cityscapes/cityscapes_vitLp16_gsp.yml', type=str, help='Config file for the experiment')
parser.add_argument('--local_rank', default=0, type=int, help='node rank for distributed training')
parser.add_argument('--run_mode', default='train', type=str, help='Config file for the experiment')
parser.add_argument('--trained_model', default=None, help='Config file for the experiment')
...
def calculate_metrics(model, input_size=(1, 3, 128, 256)):
    ...
    return params_in_million, flops_in_giga
...
def main():
    ...
    if args.run_mode != 'infer':
        for epoch in range(start_epoch, p['epochs']):
            ...
            end_signal, iter_count = train_phase(p, args, train_dataloader, test_dataloader, model, criterion, optimizer, scheduler, epoch, 
                                                 tb_writer_train, tb_writer_test, iter_count)
    ...
    if args.local_rank == 0:
        if args.run_mode == 'infer':
            ...
            eval_test = test_phase(p, args, test_dataloader, model, criterion, eval_epoch)
