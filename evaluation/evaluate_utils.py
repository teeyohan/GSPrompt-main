...
def save_model_pred_for_one_task(p, batch_idx, sample, output, save_dirs, task=None, epoch=None):
    ...
    for jj in range(int(inputs.size()[0])):
        ...
        palette_list = []
        for i in range(256):
            color = palette_dict.get(str(i), [0, 0, 0])
        ...
        if task == 'semseg':
            ...
            result.save(os.path.join(save_dirs[task], fname + '.png'))
        elif task == 'human_parts':
            result = pred.astype(np.uint8)
            result = Image.fromarray(result).convert('P')
            result.putpalette(palette_list)
            ...
            pred = pred.squeeze(-1)  # remove channel
            min_val, max_val = np.min(pred), np.max(pred)
            if max_val > min_val:  # avoid devide by zero
                pred = 255 * (pred - min_val) / (max_val - min_val)
            result = pred.astype(np.uint8)
            result = Image.fromarray(result, mode='L')
            result.save(os.path.join(save_dirs[task], fname + '.png'))
        elif task == 'edge':
