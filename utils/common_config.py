...
def get_backbone(p):
    ...
    if p['backbone'] == 'TaskPrompter_vitL':
        from models.mtpl import  taskprompter_vit_large_patch16_384
        ...
        from models.mtpl import  taskprompter_vit_base_patch16_384
...
def get_head(p, backbone_channels, task):
    ...
    if p['head'] == 'conv':
        from models.mtpl import ConvHead
        return ConvHead(backbone_channels, p.TASKS.NUM_OUTPUT[task])

    elif p['head'] == 'deconv':
        from models.mtpl import DEConvHead
...
def get_model(p):
    ...
    if p['model'] == 'TaskPrompter':
        from models.mtpl_wrapper import TaskPrompterWrapper
...
def get_train_dataset(p, transforms=None):
    ...
    if db_name == 'cityscapes':
        ...
        database = CityScapes(p.db_paths['CityScapes'], download=False, split='train', transform=transforms, 
                         do_semseg='semseg' in p.TASKS.NAMES,
...
def get_test_dataset(p, transforms=None):
    ...
    if db_name == 'PASCALContext':
        ...
        from data.cityscapes import CityScapes
        database = CityScapes(p.db_paths['CityScapes'], download=False, split='val', transform=transforms, 
                         do_semseg='semseg' in p.TASKS.NAMES,
                         do_depth='depth' in p.TASKS.NAMES)
